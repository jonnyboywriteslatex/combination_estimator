...
##  get_est - function to calculate basic estimate given in Section 2.2  of manuscript.  
##  parameters:
##  est_rt: treatment effect estimate from randomized trial
##  est_os: treatment effect estimate from observational study
##  sd_rt: standard error of treatment effect estimate from randomized trial
##  st_os: standard error of treatment effect estimate from observational study
...
##  est_cons - function to calculate basic
estimate given in Section 2.2  of
manuscript tapering the weight for the trial toward 1.  


##  parameters:


##  est_rt: treatment effect estimate from
randomized trial


##  est_os: treatment effect estimate from
observational study


##  sd_rt: standard error of treatment effect
estimate from randomized trial


##  st_os: standard error of treatment effect
estimate from observational study
...
#  fixed effects meta analysis (theta_vec,se_vec
are vectors)
...
DerSimmion/Laird estimate for between study variance (theta_vec,se_vec are
vectors)
...
  theints <- boot.ci(the.boot,conf=conf,type=c("norm","basic",
"stud", "perc")) 
...
#  Fixed-bias meta analysis (from 3.1).  The estimate for the randomized trials is
calculated using a fixed effects meta analysis. 
This function does not allow negative weights for the observational
studies
...
#  Random-bias meta analysis (from 3.1) using
DerSimmion/Laird estimate for between study variance (theta_vec,se_vec are
vectors).  The estimate for the
randomized trials is calculated using a fixed effects meta analysis.  This function does not allow negative weights
for the trials
...
##  logdata=TRUE (if the est_rt and est_os are
given on log-scale (as might be true for logistic regression).  If logdata=TRUE, effects are exponentiated before
outputing
...
##  function to convert a 95% confidence interval
(on OR scale) to the corresponding standard error on the log-OR scale
